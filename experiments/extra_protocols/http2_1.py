...
"""
class http2_1_sender( iftproto.iftsender ):
   def __init__(self):
      iftproto.iftsender.__init__(self)
      # not active sender
      self.setactive(False)
      self.job_attrs = None
      self.name = "http2_sender"
...
"""
class http2_1_receiver( iftproto.iftreceiver ):
   
   def __init__(self):
      iftproto.iftreceiver.__init__(self)
      self.name = "http2_receiver"
      self.job_attrs = None
   ...
   def get_all_attrs(self):
      return self.get_setup_attrs() + self.get_connect_attrs() + self.get_recv_attrs() + [iftproto.PROTO_PORTNUM, iftfile.JOB_ATTR_CHUNKSIZE, iftfile.JOB_ATTR_FILE_SIZE, iftfile.JOB_ATTR_FILE_HASH, HTTP_SERVER_VERSION]
   ...
   def recv_job( self, job ):
      ...
      try:
         ...
         server_version = job.get_attr( HTTP_SERVER_VERSION )
         if server_version == None or self.file_size == -1:
            if self.file_size == -1:
   ...
   def recv_chunks( self, remote_chunk_dir, desired_chunks ):
      ...
      if self.connect_args.get( USE_PARTIAL_GETS ) == False:
         if not self.job_attrs.get( iftfile.JOB_ATTR_REMOTE_IFTD ):
            if self.job_attrs.get( iftfile.JOB_ATTR_NUM_CHUNKS ) == 1:
               # use urllib2 whenever possible--it's faster
               rc, chunk_dict = self.recv_file( remote_chunk_dir, [self.job_attrs.get(iftfile.JOB_ATTR_SRC_NAME)] )
               return (rc, {0 : chunk_dict.values()[0]})
