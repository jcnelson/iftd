...
class ift_http_request_handler( SimpleHTTPRequestHandler ):
   ...
   def do_GET(self):
      ...
      try:
         ...
         if os.path.dirname( self.path.strip() ) in self.server.available_files:
            if os.path.basename( self.path.strip() ) == "done":
               # receiver is done with this file
               del available_files[ os.path.dirname( self.path.strip() ) ]
...
"""
class http3_sender( iftproto.iftsender ):
   def __init__(self):
      iftproto.iftsender.__init__(self)
      # not active sender
      self.setactive(False)
      self.job_attrs = None
      self.name = "http3_sender"
...
"""
class http3_receiver( iftproto.iftreceiver ):
   
   def __init__(self):
      iftproto.iftreceiver.__init__(self)
      self.name = "http3_receiver"
      self.job_attrs = None
      ...
      try:
         ...
         if self.file_size == -1:
            ...
            if int(fs) != int(self.file_size):
               ...
               iftlog.log(5, self.name + ": ERROR: given file size (" + str(self.file_size) + ") does not match server's file size (" + str(fs) + ")")
               return E_INVAL
      ...
      # receive chunks in fragments, since no remote IFTD and the server is HTTP/1.1
         ...
         try:
            ...
            if "multipart/byteranges" in content_type:
               ...
               t2 = time.time()
               resp.close()
               print "received chunk in " + str(t2-t1)
               bw = len(chunk_dict[chunk_id]) / (max(t2-t1, 1e-8))
               print "measured bw is " + str(bw)
               if bw > self.bandwidth:
                  print "will sleep for " + str(bw / self.bandwidth - (t2-t1))
                  time.sleep( bw / self.bandwidth - (t2-t1) )     # cap bandwidth
