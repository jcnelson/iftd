...
"""
class bittorrent2_sender( iftproto.iftsender ):
   def __init__(self):
      iftproto.iftsender.__init__(self)
      self.name = "bittorrent2_sender"
      self.file_to_send = ""
      ...
      self.set_chunking_mode( iftproto.PROTO_NONDETERMINISTIC_CHUNKING )
   ...
   def get_all_attrs(self):
      return self.get_setup_attrs() + self.get_connect_attrs() + self.get_send_attrs() + [IFTBITTORRENT_TRACKER, IFTBITTORRENT_HTTP_SEEDS, IFTBITTORRENT_TORRENT_PATH, IFTBITTORRENT_JOB_OUTPUT, IFTBITTORRENT_TORRENT_BITS]
...
"""
class bittorrent2_receiver( iftproto.iftreceiver ):
   def __init__(self):
      iftproto.iftreceiver.__init__(self)
      self.name = "bittorrent2_receiver"
      self.torrent_handle = None
      self.torrent_info = None
      self.portrange_low = 0
      self.portrange_high = 0
   ...
   def setup( self, setup_attrs ):
      iftproto.iftreceiver.setup( self, setup_attrs )
   ...
   def recv_job( self, job ):
      ...
      iftproto.iftreceiver.recv_job(self, job )
   ...
   def proto_clean( self ):
      ...
      self.torrent_info = None
      self.chunksize = None
      iftlog.log( 3, self.name + ": purging " + self.bt_dir)
      os.popen("rm -rf " + self.bt_dir ).close()
   ...
   def recv_chunks( self, remote_chunk_dir, desired_chunks ):
      # if we're a seed, we're done
      #if self.torrent_handle.is_seed():
      #   self.recv_finished( self.TRANSMIT_STATE_SUCCESS )
      #   return (0, None)

      if len(self.recv_prev) != 0:
      ...
      for chunk_id in new_chunks:
         ...
         for fs in file_slices:
            ...
            try:
               ...
               chunk_table[chunk_id] = chunk_fd.read( self.chunksize )
               chunk_fd.close()
      ...
      print "give back " + str(rc)  
      return (rc, chunk_table)
